document.addEventListener('DOMContentLoaded', function() {
  ...
  let scrollSpyObserver = null; // Holder på IntersectionObserver for ScrollSpy
  ...
  function generateTableOfContents(markdown) {
    ...
    lines.forEach(line => {
      ...
      if (match) {
        ...
        // Fjern eventuelle inline markdown-formateringer fra TOC-tittelen
    ...
    const tocHTML = tocItems.map(item => {
      return `
        <li class="toc-item toc-level-${item.level}" style="padding-left: ${(item.level - 1) * 12}px; margin-bottom: 6px; font-size: 0.9em; line-height: 1.3;">
          <a href="#${item.slug}" class="toc-link" data-toc-slug="${item.slug}" style="text-decoration: none; color: #0076d6; display: block; transition: color 0.2s;">${item.text}</a>
        </li>
      `;
    }).join('');
    ...
    return `
      <nav class="article-toc" style="flex-shrink: 0; width: 220px; position: sticky; top: 20px; align-self: start; background: #fdfdfd; padding: 15px; border-left: 2px solid #eaeaea; max-height: calc(100vh - 40px); overflow-y: auto;">
        <h4 style="margin-top: 0; margin-bottom: 12px; font-size: 0.85em; text-transform: uppercase; letter-spacing: 0.5px; color: #666;">Innhold</h4>
  ...
  function renderArticles() {
    ...
    articlesContainer.innerHTML = itemsToRender.map(article => {
      ...
      // SØKEUTDRAG (SNIPPET): Viser og markerer treff i lukket modul hvis ordet finnes dypere i teksten
      let snippetHTML = '';
      ...
      if (isExpanded) {
        ...
        // Layouten bruker en flexbox-container slik at TOC legger seg til høyre
        expandedHTML = `
    ...
    if (activeArticleId) {
      setTimeout(() => {
        ...
        initScrollSpy(); // Trigger ScrollSpy etter at DOM-en er klar og ferdig scrollet
      }, 60);
    }
  }
  // Automatisk utheving av aktive TOC-linker under rulling (ScrollSpy)
  function initScrollSpy() {
    // Rydd opp i gammel observer om den eksisterer fra forrige åpne modul
    ...
    const headings = expandedEl.querySelectorAll('.markdown-body h1, .markdown-body h2, .markdown-body h3, .markdown-body h4');
    const tocLinks = expandedEl.querySelectorAll('.toc-link');
    if (headings.length === 0 || tocLinks.length === 0) return;

    // Sørg for at overskriftene har korrekte IDer som matcher slug-genereringen
    ...
    const observerOptions = {
      root: null,
      rootMargin: '0px 0px -60% 0px', // Trigger når overskriften er i øvre halvdel av skjermen
      threshold: 0
    };
        ...
        if (entry.isIntersecting) {
          ...
          tocLinks.forEach(link => {
            if (link.getAttribute('data-toc-slug') === id) {
              ...
              link.style.borderLeft = '2px solid #0076d6';
  ...
  // Delegering av klikk på interne lenker generert fra Markdown og TOC
    ...
    articlesContainer.querySelectorAll('.filterable').forEach(articleEl => {
      ...
      if (closeBtn) {
        closeBtn.addEventListener('click', function(e) {
          e.stopPropagation();
          if (scrollSpyObserver) scrollSpyObserver.disconnect(); // Koble fra ScrollSpy ved lukking
